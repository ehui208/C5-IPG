...
// size	- 已编码的数据长度
// lawflag - A_LAW/U_LAW
static inline void G711Encoder(short *pcm, unsigned char *code, int size, int lawflag)
{
	int i;
	for( i = 0; i < size; i++ )
	{
		if( lawflag == A_LAW )
			code[i] = s16_to_alaw(pcm[i]);
		else
			code[i] = s16_to_ulaw(pcm[i]);			
	}
}
...
// size	- 已解码的数据长度
// lawflag - A_LAW/U_LAW
static inline void G711Decoder(short *pcm, unsigned char *code, int size, int lawflag)
{
	int i;
	for( i = 0; i < size; i++ )
	{
		if( lawflag == A_LAW )
			pcm[i] = alaw_to_s16(*code);
		else
			pcm[i] = ulaw_to_s16(*code);
	}
}
// lzh_20160831_e
